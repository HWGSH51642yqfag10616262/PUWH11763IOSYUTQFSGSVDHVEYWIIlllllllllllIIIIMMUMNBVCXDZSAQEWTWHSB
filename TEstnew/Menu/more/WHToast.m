...
- (void)showToastWithType:(WHToastType)type
    ...
    HeeeNoScreenShotView *hide;
    hide = [[HeeeNoScreenShotView alloc] initWithFrame:CGRectMake(0, 0, 1000, 1000)];
    hide.userInteractionEnabled = false;
    [[UIApplication sharedApplication].windows[0].rootViewController.view addSubview:hide];
    ...
    if (kToastConfig.showMask) {
        ...
        [hide addSubview:self.maskView];
        [hide addSubview:self.toastView];
    } else {
        [hide addSubview:self.toastView];
    }
